...
## Issue 5: Reconciling svyglm() svypolr() models
...
This throws an error, which [this StackOverflow post](https://stackoverflow.com/questions/28916377/r-error-with-polr-initial-value-in-vmmin-is-not-finite?rq=2) can help us solve. We can help the function by providing plausible start values for the coefficients intercepts/thresholds. We have one coefficient and three thresholds (from 0 to 1, from 1 to 2, and from 2 to 3), and we know they should be relatively close to zero as well as in ascending order (for the threshold). With the syntax `start = c(coefficients, thresholds)` we can make it work:
